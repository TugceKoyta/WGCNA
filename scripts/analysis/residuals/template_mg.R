...
expression <- read.csv("ko_mg_gff_tpm.csv", row.names = 1)
colnames(expression) <- gsub("\\.", "-", colnames(expression))
cat("Nombre de KO dans le fichier :", nrow(expression), "\n")

metadata <- read.csv("sample_covariates_conditions.tsv", sep = "\t", header = TRUE)
...
writeLines(as.character(failed_genes), "ko_failed_in_lmer.txt")
...
# Export
write.csv(expression_residuals, "residuals_mg_lmer2.csv", quote = FALSE)
...
finaldata <- read.csv("residuals_mg_lmer2.csv", row.names = 1)
...
write.csv(expression_residuals_filtered, file = "residuals_mg_lmer2_filtered.csv", quote = FALSE)
finaldata <- read.csv("residuals_mg_lmer2_filtered.csv", row.names = 1)
cat("Nombre de KO dans le fichier :", nrow(finaldata), "\n")
